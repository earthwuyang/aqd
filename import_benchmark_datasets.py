...
Adapted from ~/DB/duckdb/import_benchmark_datasets.py for current directory setup
...
# DuckDB configuration - use current directory
DUCKDB_CONFIG = {
    'binary': '/home/wuy/DB/pg_duckdb_postgres/duckdb',  # Use local duckdb binary
...
# Data directory - use current directory
DATA_DIR = '/home/wuy/DB/pg_duckdb_postgres/benchmark_data'
...
class BenchmarkDatasetImporter:
    ...
    def create_postgresql_table(self, database, table, columns, csv_path):
        """Create table in PostgreSQL and load CSV data"""
        # Create schema if not exists
        cursor = self.pg_conn.cursor()
        cursor.execute(f'CREATE SCHEMA IF NOT EXISTS "{database}"')
        
        # Create table with VARCHAR columns (simplified approach)
        column_defs = []
        for col in columns:
            # Simple type mapping - use TEXT for flexibility
    ...
    def run_with_tmux(self):
        ...
        with open('/home/wuy/DB/pg_duckdb_postgres/imported_benchmark_datasets.json', 'w') as f:
            json.dump(results, f, indent=2)
        
        print(f"\n💾 Import results saved to imported_benchmark_datasets.json")
...
def run_import_in_tmux():
    ...
    python_cmd = f"cd /home/wuy/DB/pg_duckdb_postgres && export PATH=/home/wuy/DB/pg_duckdb_postgres/postgresql_local/bin:$PATH && python3 -c 'from import_benchmark_datasets import BenchmarkDatasetImporter; importer = BenchmarkDatasetImporter(); result = importer.run_with_tmux(); print(\"Import completed with result:\", result)'"
