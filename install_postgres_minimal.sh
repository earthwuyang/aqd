#!/bin/bash
# Install PostgreSQL from source with minimal dependencies
set -e

echo "=== Installing PostgreSQL from Source (Minimal) ==="
...
if [ ! -f postgresql-16.4.tar.gz ]; then
    wget https://ftp.postgresql.org/pub/source/v16.4/postgresql-16.4.tar.gz
fi
tar -xzf postgresql-16.4.tar.gz
cd postgresql-16.4

# Configure build WITHOUT ICU, zlib, openssl, readline
echo "Configuring PostgreSQL build (minimal dependencies)..."
./configure \
...
# Initialize database with basic locale
echo "Initializing PostgreSQL database..."
mkdir -p ~/postgresql_data
$HOME/postgresql/bin/initdb -D ~/postgresql_data --encoding=UTF8 --locale=C --no-locale
