...
class ColumnPredicate:
    ...
    def __init__(self, table, col_name, operator, literal, expr=None):
        self.table = table
        self.col_name = col_name
        self.operator = operator
        self.literal = literal
        self.expr = expr  # optional full expression for the left side

    def __str__(self):
        return self.to_sql(top_operator=True)

    def to_sql(self, top_operator=False):
        left = self.expr if self.expr else f'{self.table}.{self.col_name}'
        if self.operator == Operator.IS_NOT_NULL:
            predicates_str = f'{left} IS NOT NULL'
        elif self.operator == Operator.IS_NULL:
            predicates_str = f'{left} IS NULL'
        else:
            predicates_str = f'{left} {str(self.operator)} {self.literal}'
    ...
    def generate_sql_query(self, semicolon=True):
        ...
        for i, (aggregator, columns) in enumerate(self.aggregations):
            if aggregator == Aggregator.COUNT:
                ...
                # Use a single numeric column to avoid type issues across engines
                if len(columns) == 0:
                    continue
                table, col = columns[0]
...
class APQueryGenerator:
    ...
    def sample_aggregations(self, join_tables, randstate, max_no_aggregates=3, max_cols_per_agg=1):
        ...
        for i in range(no_aggregates):
            ...
            if agg == Aggregator.COUNT:
                ...
                # Pick exactly one numeric column to keep expressions simple and valid
                col = rand_choice(randstate, numerical_columns)
                aggregations.append((agg, [col]))
