...
/* ===================================================================== *
 *  ❶  A G G R E G A T I O N   S T R U C T
 * ===================================================================== */
struct Agg15
{
  /* basic volumes & costs */
  ...
  double drSum = 0.0;       // Σ bytes read (data_read_per_join)

  /* access-type counters */
  int cRange = 0, cRef = 0, cEq = 0, cIdx = 0, cFull = 0;
  int idxUse = 0;           // USING INDEX flags

  /* plan-shape / flags */
  bool grp = false, tmp = false;
  double fanoutMax = 0.0;
  int    maxDepth  = 0;

  /* misc */
  int tables = 0;           // # primary tables visited
};


/* ===================================================================== *
 *  ❷  D F S   W A L K   (collect only what we need)
 * ===================================================================== */
static void walk(const json& n, Agg15& a, int depth = 1)
{
  if (n.is_object())
  {
    /* -------- table node -------- */
    if (n.contains("table") && n["table"].is_object())
    {
      ...
      if (ci.contains("data_read_per_join"))
      {
        ...
        dr = v.is_string() ? str_size_to_num(v.get<string>())
                           : safe_f(v);
      }
      ...
      /* fan-out ------------------------------------------------------- */
      if (re > 0)
        a.fanoutMax = std::max(a.fanoutMax, rp / re);
    }
...
/* ===================================================================== *
 *  ❸  p l a n   –––>   15-d i m   f e a t u r e   v e c t o r
 * ===================================================================== */

static bool plan2feat(const json& plan, float f[NUM_FEATS])
{
  if (!plan.contains("query_block"))
    return false;

  /* unwrap UNIONs → take first real query_block */
  ...
  Agg15 a;
  walk(*qb, a, 1);
  if (a.tables == 0) return false;

  /* ---------- global scalars ---------- */
  double qCost   = safe_f(qb->value("cost_info", json::object()), "query_cost");
  double rootRow = safe_f(*qb, "rows_produced_per_join");
  if (rootRow <= 0.0) rootRow = 1.0;         // guard / avoid div-by-0
  double tblInv  = 1.0 / a.tables;
  double mbRead  = a.drSum / 1048576.0;

  /* ---------- build vector ---------- */
  int k = 0;

  f[k++] = log1p_clip(a.re);                       // 0
  f[k++] = log1p_clip(a.rp);                       // 1
  f[k++] = float(a.rp / a.re);                     // 2  selectivity
  f[k++] = float(qCost / rootRow);                 // 3  cost per produced row
  f[k++] = float(mbRead);                          // 4  MB read total
  f[k++] = float(mbRead / rootRow);                // 5  MB per produced row

  f[k++] = a.cRange * tblInv;                      // 6  range share
  f[k++] = a.cRef   * tblInv;                      // 7  ref   share
  f[k++] = a.cFull  * tblInv;                      // 8  full  share
  f[k++] = a.idxUse * tblInv;                      // 9  using-index ratio

  f[k++] = (a.grp && a.tmp) ? 1.f : 0.f;           // 10 GROUP BY + tmp
  f[k++] = float(a.fanoutMax);                     // 11 max fan-out
  f[k++] = float(a.maxDepth);                      // 12 plan depth

  f[k++] = log1p_clip(qCost);                      // 13 log query cost
  f[k++] = (qCost > 50000.0) ? 1.f : 0.f;         // 14 cost>50k flag
...
int main(int argc, char* argv[]) {
    ...
    string modelPath     = "rf_model_succinct.json";
    double delta         = 0.2;
