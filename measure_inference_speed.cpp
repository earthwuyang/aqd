...
{
    std::string dir   = "/home/wuy/query_costs/hybench_sf1/row_plans";
    std::string model = "lgb_model_column_histogram_more.txt";
    ...
    for (int i=1;i<argc;++i){
        ...
        else if (a.rfind("--limit=",0)==0) limit = std::stoul(a.substr(8));
    }
    ...
    for (auto& p : fs::directory_iterator(dir))
        if (p.path().extension()==".json") files.push_back(p.path());
    if (files.empty()) die("no *.json found under "+dir);
    std::sort(files.begin(), files.end());
    if (files.size() > limit) files.resize(limit);
    std::cout << "[INFO] plans to test: " << files.size() << '\n';

    /* 加载模型 */
    BoosterHandle booster=nullptr; int dummy=0;
    if (LGBM_BoosterCreateFromModelfile(model.c_str(), &dummy, &booster))
        die("cannot load "+model);

    /* 批量推断 */
    long long us_sum = 0; std::size_t ok=0, skip=0;
    float  feat[NUM_FEATS]; double pred[1]; int64_t out_len=0;

    for (auto& f : files)
    {
        std::ifstream in(f); if(!in){ ++skip; continue; }
        json j; try{ in>>j; }catch(...){ ++skip; continue; }
        if(!plan2feat(j, feat)){ ++skip; continue; }

        auto t0 = hrc::now();
        /* 用普通 PredictForMat, 1×102 行即可 */
        if (LGBM_BoosterPredictForMat(
                ...
                -1, 0, "", &out_len, pred))
        { ++skip; continue; }
        auto us = std::chrono::duration_cast<std::chrono::microseconds>(hrc::now()-t0).count();
        us_sum += us; ++ok;
    }
    LGBM_BoosterFree(booster);

    if (!ok) die("all plans failed to infer");

    double avg_us = double(us_sum)/ok;
    std::cout << "\n► Inferred : " << ok
              << "\n► Skipped  : " << skip
              << "\n► Avg time : " << avg_us << " µs ("
              << avg_us/1000.0 << " ms)\n";
