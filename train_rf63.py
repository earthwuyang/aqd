...
def main():
    ...
    ap.add_argument("--data_dirs", required=True,
                    help="comma-separated list of workloads for training")
    ap.add_argument("--base", default="/home/wuy/query_costs")
    ap.add_argument("--trees", type=int, default=32)
    ap.add_argument("--max_depth", type=int, default=15)
    ap.add_argument("--min_samples", type=int, default=40)
    ap.add_argument("--sample_ratio", type=float, default=0.6)
    ...
    # ---------------------------------------------------------------- data
    X, y, row_t, col_t, qCost = load_dataset(base_dir, data_dirs)
    log(f"Loaded {len(X)} samples")

    # ---------------------------------------------------------------- weights
    P = y.sum(); N = len(y)-P
    w1 = len(y)/(2*P)  if P else 1.0
    w0 = len(y)/(2*N)  if N else 1.0
    gap = np.abs(row_t-col_t)
    wgap  = 1 + np.log1p(gap)                 # smoother than original
    wcost = 1 + np.minimum(qCost,1e6)/1e5
    sample_w = np.where(y==1, w1, w0) * wgap * wcost

    # ---------------------------------------------------------------- model
    if args.skip_train and Path(args.model).exists():
        ...
        dump(rf, args.model)
        log(f"Model saved  →  {args.model}")

    # ---------------------------------------------------------------- evaluate (on train for parity)
    preds = rf.predict(X)
    ...
    # runtime comparison identical to C++ summary
    delta = 0.2
    costThreshold=5e4
    probs = rf.predict_proba(X)[:,1]
    dec   = np.where(np.abs(probs-0.5)<delta,
                     qCost>costThreshold,
                     probs>=0.5)
    rt_rf   = np.mean(np.where(dec, col_t, row_t))
    rt_row  = np.mean(row_t)
    rt_col  = np.mean(col_t)
    rt_rule = np.mean(np.where(qCost>costThreshold, col_t, row_t))
