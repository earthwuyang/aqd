...
def build_mixed_mismatch(
    ...
    for ds in datasets:
        ...
        base += [(ds, q) for q in qs]
    random.shuffle(base)
    topk = min(len(base), args.limit)
    heavy_part = base[:topk]
    rest = base[topk:]
    return heavy_part + rest
...
class UtilMonitor(Thread):
    ...
    def _fetch_row_col_bytes(self, cur) -> tuple[int, int]:
        ...
        col_bytes  = exec_mem(若非 0) + max(lru - base,0) —— 列存
        """
        # —— ROW (InnoDB) ───────────────────────────────
        ...
        lru = int(cur.fetchone()[1])

        if self._lru_base is None:          # 第一次采样：记录基准
            self._lru_base = lru
        ...
        # 如果 exec_mem 为 0，则退化为 LRU 的增长量
        lru_delta = max(lru - self._lru_base, 0)

        col_bytes = exec_mem + lru_delta
...
def main():
    ...
    pa.add_argument("--blocker_threads", type=int, default=10)  # NEW: multiple blocker threads to overload row
    ...
    # Force scenario settings for DR UCB advantage
    args.AP_heavy = True  # Load AP-heavy queries (column faster)
    args.scenario = "resource_spike"  # Use resource_spike for high-concurrency spikes and blockers
    args.burst_qps = 1200  # High QPS in bursts to overload
    args.burst_len = 300  # Longer bursts
    args.pause_ms = 100  # Shorter pauses for sustained pressure
    args.limit = 500  # Fewer queries for faster benchmarking
    args.rounds = 5  # More rounds for stable averages
    args.blocker_repeat = 20  # More repeats per blocker
    args.blocker_threads = 20  # More threads to heavily overload row
    ...
    if args.scenario == "resource_spike":
        ...
        blocker_threads = []
        for _ in range(args.blocker_threads):
            th = Thread(
            ...
            th.start()
            blocker_threads.append(th)
        time.sleep(3)  # give blockers time to start and overload row engine
    ...
    for rd in range(1, args.rounds + 1):
        ...
        print("\n------------------------------------------------------------")
        time.sleep(10)  # Allow system to recover between rounds
