...
int main(int argc,char*argv[])
{
    ...
    std::string csv = "problem_sqls.csv";

    /* ---------- 可选 CLI 覆盖 ---------- */
    for (int i=1;i<argc;++i) {
        std::string a(argv[i]);
        if (a.rfind("--csv=",0)==0) csv = a.substr(6);
    }

    /* ---------- 1) 载入 3 个 LightGBM 模型 ---------- */
    std::vector<BoosterHandle> boosters;
    for (const auto& m : models) {
        BoosterHandle h=nullptr; int iters=0;
        if (LGBM_BoosterCreateFromModelfile(m.c_str(), &iters, &h)) {
            logE("cannot load "+m);  return 1;
        }
        boosters.push_back(h);
        logI("loaded model "+m);
    }

    /* ---------- 2) 读取 CSV ---------- */
    std::ifstream fin(csv);
    if(!fin){ logE("cannot open "+csv); return 1; }
    ...
    while(read_csv_row(fin,row) && row.size()>=10){
        ...
        if(!r.db.empty()){ Q.push_back(r); dbs.insert(r.db); }
    }
    logI("loaded "+std::to_string(Q.size())+" queries from CSV");

    /* ---------- 3) 预加载索引 (可选) ---------- */
    load_all_index_defs("127.0.0.1", 3306, "root", "",          /* 修改为你的 MySQL 连接 */
    ...
    for(const auto& r:Q)
    {
        /* —— 生成 plan.json —— (此处示意：若你已有 plan 文件可直接读) */
        /*    若要在线获取，可参考前面版本用 EXPLAIN FORMAT=JSON          */
        std::string plan_file = r.db+"/row_plans/"+r.qid+".json";
        std::ifstream pin(plan_file);
        if(!pin){ logW("plan missing "+plan_file); continue; }
        json plan; pin>>plan;

        float feat[NUM_FEATS]; if(!plan2feat(plan,feat)){
            logW("plan2feat failed "+r.qid); continue;
        }

        /* —— 每个模型都预测一次 —— */
        int votes = 0;
        for(auto h:boosters){
            double score=0; int64_t out_len=0;
            LGBM_BoosterPredictForMat(h, feat, C_API_DTYPE_FLOAT32,
                                      1, NUM_FEATS, 1, C_API_PREDICT_NORMAL,
                                      -1, 0, "", &out_len, &score);
            if(score > 0.0) ++votes;            // “列存” 票
        }
        bool pred_col = (votes >= 2);           // 简单多数 (3→2)

        bool actual_col = (r.use_imci==1);
        if(pred_col == actual_col) ++correct;
        ++total;

        std::cout<<r.qid<<"  pred="<<(pred_col?"COLUMN":"ROW")
                 <<"  actual="<<(actual_col?"COLUMN":"ROW")
                 <<"  votes="<<votes<<"\n";
    }
    ...
    /* ---------- 5) 资源释放 ---------- */
    for(auto h:boosters) LGBM_BoosterFree(h);
    return 0;
}
