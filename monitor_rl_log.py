...
def create_visualization(stats: Dict, output_file: str = "rl_residual_learning.png"):
    ...
    # Plot only residual learning signals
    plt.plot(x, stats['rewards'], '-', alpha=0.35, linewidth=0.9, label='Immediate Reward')
    plt.plot(x, stats['ema'], '-', linewidth=2.0, label='Smoothed Reward (EMA)')
    plt.axhline(y=0, color='k', linestyle='--', alpha=0.4, linewidth=0.8)

    plt.xlabel('Query #')
    plt.ylabel('Reward Signal')
    plt.title('RL Residual Learning Progress', fontsize=14, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"Residual learning visualization saved to {output_file}")
