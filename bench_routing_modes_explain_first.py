...
# 计划 → ROW/COL 判定结果缓存，避免同一条 SQL 每次都 EXPLAIN
# key = hash(sql_text)  value = "ROW" / "COL"
...
def _decide_imci(plan_json: str) -> int:
    """
    EXPLAIN FORMAT='json' 得到的 plan 字符串 → 0(ROW) / 1(COL)

    PolarDB-MySQL 的列存计划不会出现 "query_block"，
    行存计划一定包含 "query_block" 字段。
    """
...
def _explain_and_decide(cur, sql: str) -> str:
    """
    首次用 EXPLAIN 判断行/列；结果写入 _plan_cache。
    返回值 'ROW' / 'COL'
    """
    key = hash(sql)
    with _cache_lock:
...
def execute_query(
    ...
    try:
        ...
        # 3. **EXPLAIN 选资源组**（只有打开 --enable_resource_control 时才做）
        if getattr(args, "enable_resource_control", False):
            eng = _explain_and_decide(cur, sql)     # 'ROW' / 'COL'
