...
//===--------------------------------------------------------------------===//
// Query Routing Method Setting
//===--------------------------------------------------------------------===//
// Query Routing Method Setting
//===--------------------------------------------------------------------===//
void QueryRoutingMethodSetting::SetGlobal(DatabaseInstance *db, DBConfig &config, const Value &input) {
	auto new_method = input.ToString();
	if (new_method != "cost_threshold" && new_method != "ml_model") {
		throw InvalidInputException("Query routing method must be 'cost_threshold' or 'ml_model'");
	}
	config.options.query_routing_method = new_method;
}

void QueryRoutingMethodSetting::ResetGlobal(DatabaseInstance *db, DBConfig &config) {
	config.options.query_routing_method = "cost_threshold";
}

Value QueryRoutingMethodSetting::GetSetting(const ClientContext &context) {
	auto &config = DBConfig::GetConfig(context);
	return Value(config.options.query_routing_method);
}
