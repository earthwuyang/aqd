...
unique_ptr<QueryResult> ClientContext::Query(const string &query, bool allow_stream_result) {
	...
	for (idx_t i = 0; i < statements.size(); i++) {
		...
		// We only compute and log a routing decision here; execution continues as usual.
		// This keeps correctness while validating routing logic end-to-end.
		try {
			// Initialize router on first use
			InitializeQueryRouter(*this);
			if (IsQueryRoutingEnabled() && statement) {
				// Build a logical plan to extract features for routing
				// Note: this incurs extra planning work but does not alter execution.
				auto plan_for_routing = ExtractPlan(statement->query);
				if (plan_for_routing) {
					// Extract features using the feature logger if available
					QueryFeatures features;
					if (g_query_feature_logger) {
						features = g_query_feature_logger->ExtractFeatures(*plan_for_routing, statement->query);
					}
					// Make routing decision
					auto decision = g_query_router->MakeRoutingDecision(*plan_for_routing, features, statement->query);
					// Emit a concise note for observability (stderr to avoid interfering with result piping)
					std::cerr << "[router] decision="
					          << (decision == RoutingDecision::ROUTE_TO_DUCKDB ? "duckdb" : "postgresql")
					          << " tables=" << features.num_tables
					          << " joins=" << features.num_joins
					          << " aggs=" << features.num_aggregates
					          << std::endl;
				}
			}
		} catch (...) {
