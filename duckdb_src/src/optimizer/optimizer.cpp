...
unique_ptr<LogicalOperator> Optimizer::Optimize(unique_ptr<LogicalOperator> plan_p) {
	...
	// Log query features if enabled
	if (g_query_feature_logger && g_query_feature_logger->IsEnabled()) {
		// Get query text from context if available
		string query_text = "";
		// TODO: Get actual query text from context when available
		
		// Extract and log features
		auto features = g_query_feature_logger->ExtractFeatures(*plan, query_text);
		g_query_feature_logger->LogFeatures(features);
	}
