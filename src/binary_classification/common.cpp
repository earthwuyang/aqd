...
static void load_features_for_dir(const std::string& dir)
{
    std::string csv = dir + "/features_140d.csv";
    if (!file_exists(csv)) {
        logW("features_140d.csv not found in " + dir);
        return;
    }

    /* ------------- 140→32 的下标映射表（-1 表示本行不要） ------------- */
    static int map140to32[140];
    static bool once = false;
    if (!once) {               // 只在第一次调用时初始化
        std::fill_n(map140to32, 140, -1);
        for (int k = 0; k < NUM_FEATS; ++k)
            map140to32[ TOP32_IDX[k] ] = k;
        once = true;
    }
    ...
    while (std::getline(fin, line))
    {
        ...
        /* —— 2) 原始 140 维逐个扫描，只拾需要的 32 维 —— */
        FeatVec fv{};                // NUM_FEATS = 32
        for (int i = 0; i < 140; ++i) {
            std::getline(ss, tok, ',');
            int pos = map140to32[i];
            if (pos >= 0) {          // 这维在 TOP32 列表里
                fv[pos] = tok.empty() ? 0.f : std::stof(tok);
            }
        }
        /* (忽略剩余 SQL 列) */
    ...
    logI("Loaded feature cache (32-d) for " + dir);
}
