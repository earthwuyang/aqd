...
constexpr const char* ROW_SUFFIX = "_row";
constexpr const char* COL_SUFFIX = "_col";
...
inline bool model_ready(const std::string& base)
{
    return file_exists(base + ROW_SUFFIX) && file_exists(base + COL_SUFFIX);
}

/* ───── qerror reporter for an on-disk model ───────────────────────── */
static void report_qerr_file(const std::string& model_base,
                             const std::vector<Sample>& samp)
{
    ...
    for (int64_t i = 0; i < n; ++i)
        std::copy(samp[i].feat.begin(),
                  samp[i].feat.end(),
                  X.begin() + size_t(i) * F);

    auto one_side = [&](const char* tag, bool is_row)
    {
        /* 1. 读模型文件 → booster */
        BoosterHandle h = nullptr;
        const std::string fpath = model_base + (is_row ? ROW_SUFFIX : COL_SUFFIX);
        int num_iter = 0;
        chk(!LGBM_BoosterCreateFromModelfile(fpath.c_str(), &num_iter, &h),
            "LoadModel");
        ...
        const double p95  = qerr[ size_t(0.95 * (n - 1)) ];
    ...
    one_side("[row]", /*is_row=*/true);
    one_side("[col]", /*is_row=*/false);
}
...
int main(int argc, char* argv[])
{
    ...
    if (!query_file.empty()) {
        ...
        report_qerr_file(model_path, DS);   // ← 新增
    ...
    if (hp.skip_train) {
        ...
        report_qerr_file(model_path, DS_test);   // ← 新增
    ...
    * ------------------------------------------------------------ */
    if (data_dirs.size() == 1 && !mix_folds)
    {
        ...
        report_qerr_file(model_path, DS_te);   // ← 新增
        return 0;                             // 整个程序在此结束
    }
    ...
    {
        /* 1) best_src 只是“公共前缀”（不带 _row/_col） */
        const std::string best_src = fold_scores.front().second;          // e.g. checkpoints/lightgbm_fold3.txt
        const std::string best_dst = "checkpoints/" + model_type + "_best.txt";

        /* 2) 实际需要复制两个文件：<prefix>_row  和  <prefix>_col          */
        const std::string src_row = best_src + "_row";     // checkpoints/lightgbm_fold3.txt_row_fold
        const std::string src_col = best_src + "_col";     // checkpoints/lightgbm_fold3.txt_col_fold
        const std::string dst_row = best_dst + "_row";     // checkpoints/lightgbm_best.txt_row
        const std::string dst_col = best_dst + "_col";     // checkpoints/lightgbm_best.txt_col

        /* 3) 使用 cp -f，省掉 <filesystem> 依赖                                */
        auto do_copy = [](const std::string& src, const std::string& dst){
            std::string cmd = "cp -f " + src + " " + dst;
            if (std::system(cmd.c_str()) != 0)
                throw std::runtime_error("cp failed: " + src + " → " + dst);
        };

        try {
            do_copy(src_row, dst_row);
            do_copy(src_col, dst_col);
            logI("saved best model → " + best_dst + "_{row,col}");
    ...
    report_qerr_file("checkpoints/" + model_type + "_best.txt", DS_test);   // ← 新增
    return 0;
}
