...
RoutingDecision LightGBMRouter::PredictRouting(const QueryFeatures &features) {
    ...
    try {
        ...
        // Make prediction
        std::vector<double> prediction_results(3);  // For 3-class prediction (postgresql, duckdb, postgres_query)
        ...
        if (result != 0 || out_len <= 0) {
            std::cerr << "[LightGBMRouter] Prediction failed (result=" << result 
                     << ", out_len=" << out_len << "), using fallback" << std::endl;
            goto fallback;
        }
        
        // Convert multiclass probabilities to routing decision
        // Classes: [0=postgresql, 1=duckdb, 2=postgres_query]
        RoutingDecision decision;
        if (out_len == 1) {
            // Binary classification (legacy model)
            decision = (prediction_results[0] > prediction_threshold) ?
            ...
            int best_class = 0;
            double best_prob = prediction_results[0];
            for (int i = 1; i < 3; i++) {
                if (prediction_results[i] > best_prob) {
                    best_prob = prediction_results[i];
                    best_class = i;
                }
            }
            
            switch (best_class) {
                case 0: decision = RoutingDecision::ROUTE_TO_POSTGRESQL; break;
                case 1: decision = RoutingDecision::ROUTE_TO_DUCKDB; break;
                case 2: decision = RoutingDecision::ROUTE_TO_POSTGRES_QUERY; break;
                default: decision = RoutingDecision::ROUTE_TO_DUCKDB; break;
            }
        } else {
            std::cerr << "[LightGBMRouter] Unexpected prediction length: " << out_len << std::endl;
            goto fallback;
        }
...
bool EnhancedQueryRouter::InitializeLightGBM(const string &model_dir) {
    ml_router = std::make_unique<LightGBMRouter>();
...
RoutingDecision EnhancedQueryRouter::MakeRoutingDecision(const LogicalOperator &plan,
                                                        const QueryFeatures &features,
                                                        const string &query_text) {
    // First check for manual hints (inherited from base class)
...
string EnhancedQueryRouter::GetRoutingStats() const {
    string stats = QueryRouter::GetRoutingStats();
    
    if (use_ml_routing && ml_router) {
        stats += "\n" + ml_router->GetModelInfo();
    }
...
unique_ptr<LightGBMRouter> CreateLightGBMRouter(const string &model_dir) {
    auto router = std::make_unique<LightGBMRouter>();
...
bool LoadQueryRoutingModel(const string &model_dir) {
    g_enhanced_router = std::make_unique<EnhancedQueryRouter>();
...
bool IsMLRoutingAvailable() {
    return g_enhanced_router && g_enhanced_router->use_ml_routing;
}
