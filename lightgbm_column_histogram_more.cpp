...
int main(int argc,char*argv[]){
    // string base="/home/wuy/query_costs", model="lgb_model_column_histogram_more.txt";
    string base="/home/wuy/query_costs", model="lgb_model_tmp.txt";
    vector<string> dirs;
    ...
    if (!populate_col_stats(host, port, user, pass, dirs))
        std::cerr << "[WARN]  proceeding with default column stats\n";



    if(!populate_tbl_stats(host,port,user,pass,dirs))
        logW("table stats fallback to zeros");
