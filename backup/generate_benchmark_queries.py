...
class APQueryGenerator(QueryGenerator):
    ...
    def _build_where_clause(self, tables):
        """Build WHERE clause with consistent and logical predicates"""
        predicates = []
        used_columns = set()  # Track columns to avoid conflicts
        ...
        for _ in range(num_predicates):
            ...
            # Find unused column to avoid conflicts
            available_cols = [col for col in cols if f"{table}.{col}" not in used_columns]
            ...
            # Get column nullability from schema info
            table_cols = self.schema_info['tables'].get(table, [])
            is_nullable = True  # Default to nullable
            for col_name, data_type, nullable, default in table_cols:
                if col_name == col:
                    is_nullable = (nullable == 'YES')
                    break
            
            # Build predicate based on datatype
            predicate = None
            ...
            if dtype in [Datatype.INT, Datatype.FLOAT]:
                ...
                if literal is not None:
                    predicate = f'"{table}"."{col}" {op} {literal}'
                ...
                if random.random() < 0.7:
                    ...
                    # Use LIKE operator with meaningful patterns
                    pattern = random.choice(["a", "e", "i", "o", "u", "s", "t", "n", "r"])
                    predicate = f'"{table}"."{col}" LIKE \'%{pattern}%\''
            
            elif dtype == Datatype.BOOLEAN:
                predicate = f'"{table}"."{col}" = {random.choice(["TRUE", "FALSE"])}'
            
            elif dtype in [Datatype.DATE, Datatype.TIMESTAMP]:
                # For date columns, use comparison operators
                op = random.choice([Operator.GT, Operator.LT, Operator.GEQ, Operator.LEQ])
                # Generate reasonable date literals
                if dtype == Datatype.DATE:
                    date_literal = f"'{random.randint(1990, 2023)}-{random.randint(1, 12):02d}-{random.randint(1, 28):02d}'"
                    predicate = f'"{table}"."{col}" {op} {date_literal}'
                else:
                    timestamp_literal = f"'{random.randint(1990, 2023)}-{random.randint(1, 12):02d}-{random.randint(1, 28):02d} 00:00:00'"
                    predicate = f'"{table}"."{col}" {op} {timestamp_literal}'
            
            # Only add NULL checks for nullable columns and with low probability
            if is_nullable and random.random() < 0.1:
                null_op = random.choice([Operator.IS_NULL, Operator.IS_NOT_NULL])
                # Replace the regular predicate with NULL check (avoid conflicts)
                predicate = f'"{table}"."{col}" {null_op}'
            
            if predicate:
                predicates.append(predicate)
        
        if predicates:
            # Use only AND to avoid contradictory logic
    ...
    def _build_where_clause(self, tables):
        ...
        for table in tables:
            ...
            if pks and random.random() < 0.7:
                # Use primary key
                for pk in pks:
                    if f"{table}.{pk}" not in used_columns:
                        stats = self.column_stats.get(table, {}).get(pk, {})
                        literal = self.sample_literal(stats)
                        if literal is not None:
                            predicates.append(f'"{table}"."{pk}" = {literal}')
                            used_columns.add(f"{table}.{pk}")
                            break
            else:
                # Use high-cardinality column
                high_card_cols = []
                for col, stats in self.column_stats.get(table, {}).items():
                    if f"{table}.{col}" not in used_columns and stats.get('unique_count', 0) > 100:
        ...
        # Add additional filters (but avoid conflicts)
        if random.random() < 0.3:
            table = random.choice(tables)
            cols = list(self.column_stats.get(table, {}).keys())
            available_cols = [col for col in cols if f"{table}.{col}" not in used_columns]
