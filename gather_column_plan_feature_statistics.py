...
def main():
    ...
    col_plans_dir = os.path.join(data_dir, 'column_plans')
    workload_run_paths = [os.path.join(col_plans_dir, f) for f in os.listdir(col_plans_dir) if f.endswith('.json')]
    target = os.path.join(data_dir,'column_plan_statistics.json')
