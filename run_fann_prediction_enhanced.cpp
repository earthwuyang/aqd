...
// • Extracts the **same 24-dimensional feature vector** used during training
...
// • Feeds the scaled vector into the 24 → hidden → 1 FANN network
...
/* names that appear in row_plan_statistics.json (leave nullptr for
   “derived” features that use hard caps)                                      */
static const char* STAT_NAME[NUM_FEATS] = {
  ...
  /* 12   */ nullptr,                       // using-index fraction
  /* 13-15*/ "filtered","filtered","filtered",   // selectivity stats surrogate
  /* 16-17*/ nullptr,nullptr,               // depth / fan-out
  /* 18-20*/ nullptr,nullptr,nullptr,       // boolean flags
  /* 21-22*/ "read_cost","read_cost",       // ratio mean & max
  /* 23   */ "query_cost"
};

/* pessimistic hard caps used if stats file is missing or key not present */
...
/* load min/max   (we only need the “max” column – min is 0 for all logged vals) */
static void loadScaling()
{
    ...
    if (!fileExists(statPath)) {
        ...
        scalingLoaded = true; return;
    }

    json st;
    try { std::ifstream(statPath) >> st; }
    catch (...) { logWarn("stats JSON parse error – using defaults"); scalingLoaded = true; return; }
...
static float scaleFeat(int idx, double v)
{
    ...
    if (v < featMin[idx]) v = featMin[idx];
    if (v > featMax[idx]) v = featMax[idx];
...
double convert_data_size_to_numeric(std::string s)
{
    ...
    if (suf=='G'){ f=1e9; s.pop_back();}
    else if (suf=='M'){ f=1e6; s.pop_back();}
    else if (suf=='K'){ f=1e3; s.pop_back();}
...
void recursePlan(const json &node, FeatureAgg &A, int depth)
{
    if (node.is_object()) {
        if (node.contains("table") && node["table"].is_object()) {
            ...
            json ci = tbl.contains("cost_info") ? tbl["cost_info"] : json::object();
...
/* fill raw feature vector (UNscaled) */
...
{
    /* ----------  database connection ---------- */
    const char *host="127.0.0.1", *user="root", *pass="", *db="task_info";
    ...
    const char *qry =
        "EXPLAIN FORMAT=JSON "
        "SELECT count(*) AS count, module_code "
        "FROM task_info "
        "WHERE (env='online' "
